"""Read-only database inspection for agents (DatabaseClient only).

Local dev::

    doppler run --project chess-teacher --config dev_local -- ^
      .venv\\Scripts\\python.exe scripts/tools/agent_db_query.py --json list-domains

Prod (via VPS skill ``db-*`` commands) runs the same script inside the
streamlit container: ``kubectl exec … -- python scripts/tools/agent_db_query.py …``
"""
...
def validate_table_key(table: str) -> str:
    text = table.strip()
...
def validate_column_list(columns: str) -> str:
...
def cmd_list_tables(args: argparse.Namespace) -> None:
    domain = validate_domain_id(args.domain)
    keys = list_table_keys(domain)
    emit(
        {
            "domain": domain,
            "metadata_path": _path_for_display(resolve_domain(domain)),
            "tables": keys,
        },
        as_json=args.json,
    )
...
def cmd_read(args: argparse.Namespace) -> None:
    domain = validate_domain_id(args.domain)
    table_key = validate_table_key(args.table)
    table = load_table(domain, table_key)
    where = validate_sql_fragment(args.where, label="WHERE") if args.where else None
    columns = None
    if args.columns:
        columns = [c.strip() for c in validate_column_list(args.columns).split(",")]
        validate_columns(table, columns)
    order_by = validate_sql_fragment(args.order_by, label="ORDER BY") if args.order_by else None
...
def cmd_unique(args: argparse.Namespace) -> None:
    ...
    table_key = validate_table_key(args.table)
    table = load_table(domain, table_key)
    columns = [c.strip() for c in validate_column_list(args.columns).split(",") if c.strip()]
    if not columns:
