...
class LoadRawLogsStep(StorageToTableStep):
    ...
    def _load_records(self, db_client: DatabaseClient, context: PipelineContext) -> pl.DataFrame:
        ...
        file_total = len(keys)
        context.progress_update(
            f"Found {file_total} log file{'s' if file_total != 1 else ''} to load."
        )
        records: list[dict] = []
...
class DeleteOldS3LogFilesStep(PipelineStep):
    ...
    def run(self, db_client: DatabaseClient, context: PipelineContext) -> None:
        ...
        keys = storage.list_keys(
            self.CLOSED_LOG_STORAGE_PREFIX,
            recursive=True,
            suffix=self.LOG_FILE_SUFFIX,
        )
        ...
        to_delete: list[str] = []
        for key in keys:
            relative = ObjectStorage.relative_key_under(key, self.CLOSED_LOG_STORAGE_PREFIX)
            path_date = self.parse_closed_log_path_date(relative)
